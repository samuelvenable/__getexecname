...
It supports Windows, macOS, Linux, GNU/Hurd, Cygwin, FreeBSD, DragonFly BSD, NetBSD, OpenBSD, Solaris, illumos, QNX, QNX Neutrino, Haiku, and Android. iOS, iPadOS, tvOS, watchOS, and visionOS are supported platforms as well, though to a limited extent, because only getting the executable path name from the current process is supported on those platforms, and not the executable path name of an external PID; iOS, iPadOS, tvOS, watchOS, and visionOS are the only targets with this issue.
