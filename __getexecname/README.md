...
It supports Windows, macOS, GNU/Linux, GNU/Hurd, FreeBSD, DragonFly BSD, NetBSD, OpenBSD, Solaris, illumos, QNX, QNX Neutrino, Haiku, and Android. Cygwin, iOS, iPadOS, tvOS, watchOS, and visionOS are supported platforms as well, though to a limited extent, because only getting the executable path name from the current process is supported on those platforms, and not the executable path name of an external PID; Cygwin, iOS, iPadOS, tvOS, watchOS, and visionOS are the only targets with this issue.

`__getexecname()` has more features, is more accurate, less erroneous, and more reliable, than most of its alternatives, ones such as [ibara/getexecname](https://github.com/ibara/getexecname)'s [getexecname()](https://github.com/ibara/getexecname) function for OpenBSD, [gpakosz/whereami](https://github.com/gpakosz/whereami)'s cross-platform function [wai_getExecutablePath()](https://github.com/gpakosz/whereami/tree/master/src), [cginternals/cpplocate](https://github.com/cginternals/cpplocate)'s cross-platform function [getExecutablePath()](https://github.com/cginternals/cpplocate/tree/master/source/liblocate/source), [boostorg/process](https://github.com/boostorg/process)'s cross-platform function [exe()](https://github.com/boostorg/process/tree/develop/src/ext), [boostorg/dll](https://github.com/boostorg/dll)'s cross-platform function [program_location()](https://github.com/boostorg/dll/tree/develop/include/boost/dll), [libsdl-org/SDL](https://github.com/libsdl-org/SDL)'s cross-platform function [SDL_GetBasePath()](https://github.com/libsdl-org/SDL/tree/main/src/filesystem), and [raysan5/raylib](https://github.com/raysan5/raylib)'s cross-platform function [GetApplicationDirectory()](https://github.com/raysan5/raylib/tree/master/src). Feel free to compare source code and make various tests if you are in any doubt.
...
`__getbasepath()` follows the same exact code logic as `__getexecname()`, except instead of returning the absolute path name to the executable file, it only returns the absolute path name to the executable file's parent directory. The `__getbasepath()` function was inspired by [libsdl-org/SDL](https://github.com/libsdl-org/SDL)'s [SDL_GetBasePath()](https://github.com/libsdl-org/SDL/tree/main/src/filesystem) function, and [raysan5/raylib](https://github.com/raysan5/raylib)'s [GetApplicationDirectory()](https://github.com/raysan5/raylib/tree/master/src) function, which effectively serve the same exact purpose, in principle.
